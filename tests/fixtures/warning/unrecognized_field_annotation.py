"""Warning fixture: an unrecognized field annotation degrades loudly (pplw-unrecognized-annotation).
...
referencing the schema gets a pplw-unrecognized-annotation warning instead.

Why a warning and not a pple-broken-schema-annotation error: the bare name may be a runtime alias
...
class WithMystery(pa.DataFrameModel):
    a: int
    blob: CustomPayload  # unrecognized -> Unknown dtype column + pplw-unrecognized-annotation
...
def keeps_unknown_column(df: DataFrame[Src]) -> DataFrame[WithMystery]:
    ...
    # diagnostic is the pplw-unrecognized-annotation advisory on the schema definition.
